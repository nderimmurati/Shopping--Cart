...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Kërkohet emri i përdoruesit"); }
  if (empty($email)) { array_push($errors, "Kërkohet email"); }
  if (empty($password_1)) { array_push($errors, "Kërkohet fjalëkalimi"); }
  ...
  if ($user) { // if user exists
    if ($user['Name'] === $username) {
      array_push($errors, "Emri i përdoruesit tashmë ekziston");
    }
  ...
  if (count($errors) == 0) {
  	...
  	$_SESSION['success'] = "Tani jeni regjistruar";
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Kombinim i gabuar i emrit të përdoruesit / fjalëkalimit");
  	}
  }
}
